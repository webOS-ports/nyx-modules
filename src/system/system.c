...
/*
 * Name a wakeup source that is just a number.
 *
 * Kernels with the Android per-IRQ wakeup sources register one named after the
 * IRQ number, so the log line reads "active: 112" and says nothing. Resolve it
 * through /proc/interrupts, where the last field is the action name: "112
 * (vrd-change)". Leaves anything non-numeric alone.
 */
static void name_numeric_source(const char *token, char *out, size_t len)
{
	char *table = NULL;
