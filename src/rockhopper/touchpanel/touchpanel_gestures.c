...
int gesture_state_machine_finger(finger_t *finger, input_event_t *events,int *numEvents);
...
void
update_coord_buffer(coord_buf_t* pCoordBuf, int xCoord, int yCoord, const time_stamp_t* pTime)
{
    if (pCoordBuf->head == pCoordBuf->tail && pCoordBuf->numItems == pCoordBuf->size)
    {
...
void get_last_coords(const coord_buf_t *pCoordBuf,int *xCoord,int *yCoord,time_stamp_t *timestamp)
{
...
//We have set minDistId to the index of the finger in the input array
    ...
    for(j = 0; j < numFingers; j++)
    {
        ...
        if(pFingerWeights[j] < g_atomic_int_get(&spGeneralSettings->fingerDownThreshold)) {
            nyx_info("Discarding finger with too low weight (%d)\n",pFingerWeights[j]);
            continue;
        }
        nyx_info("pFingerWeight: %d (%d)\n",pFingerWeights[j],j);

        nyx_info("j: %d, %d) New finger @ %d,%d\n",j,numFingers,pXCoords[j],pYCoords[j]);
...
{ 
    ...
    set_event_params(&finger->events[finger->numEvents++], &timestamp, EV_FINGERID, 0 , finger->id);
    ...
    case START_STATE:
    {
        ...
        set_event_params(&finger->events[finger->numEvents++], &timestamp, EV_KEY, BTN_TOUCH, 1);
    }
    break;
    ...
    set_event_params(&finger->events[finger->numEvents++], &timestamp, EV_ABS, ABS_X, x);
